...
| 2026-09-14 | Sécurité des dépendances : Next 15.5.19 → 15.5.25 (RCE Windows, RCE AVIF, SSRF, DoS, cache), overrides npm `postcss` ≥ 8.5.28 et `fflate` ≥ 0.8.3, `sharp` 0.35.4 ; Spring Boot 3.2.5 → 3.5.16 (Tomcat, Spring Security, Spring Framework, Jackson, Netty), Flyway 11 + module PostgreSQL, JJWT 0.12.7, logstash-logback-encoder 8.1 ; images Docker sur Alpine (`eclipse-temurin:21-jre-alpine`, `node:22-alpine`), `apk upgrade`, utilisateur non-root, `.dockerignore`, frontend en sortie Next « standalone » sans npm ; Tomcat 10.1.59, Netty 4.1.138, PostgreSQL 42.7.13, Jackson 2.21.6, Log4j API 2.25.5 épinglés après analyse Docker Scout. Curseur : fleur seule, 28 px, halo retiré. | `npm audit` : 0 vulnérabilité ; 40 tests backend verts sur Boot 3.5.16 ; build frontend vert. Docker Scout : backend 110 → 4 (0 critique / 0 élevée, correctifs Alpine non publiés), frontend 19 → 0 ; images 466 Mo et 328 Mo. Restant : `stripe-java` 25 → 33 (changement majeur d'API, à planifier), WebAuthn4J 0.22 → 0.31 (API refondue). |
| 2026-09-14 | Bibliothèques backend au plus récent : Stripe 33.4.2, WebAuthn4J 0.31.10 (API dépréciée migrée), JJWT 0.13.0, Logstash encoder 9.0, Jackson 2.22.2. Deux défauts trouvés par le nouveau test : incompatibilité jackson-annotations (WebAuthn4J sur Jackson 3) et connexion passkey qui échouait hors transaction (utilisateur chargé paresseusement, corrigé par `@EntityGraph`). | `WebAuthnFlowTest` (enregistrement + connexion émulés, défi à usage unique, signature altérée) ; 42 tests verts. |
| 2026-09-14 | Paquets frontend au plus récent de leur majeure (TanStack Query 5.102, Lenis 1.3, lucide-react 1.46, Framer Motion 11.18, Zustand 4.5.7, TypeScript 5.9, ESLint 9.39, Tailwind 3.4.19, @types/node 22) ; `clsx` et `tailwind-merge` retirés ; icône YouTube locale (logos absents de lucide 1.x) ; typage `Uint8Array<ArrayBuffer>` (TypeScript 5.9). Majeures React 19 / Next 16 / R3F 9 / Tailwind 4 reportées en une vague dédiée. | typecheck, lint, build verts ; `npm audit` à zéro. |
